...
the PHP application. `embedded`, `thumbnail`, and `original` jobs are queued on
ingest. `preview`, `final`, and `rawtherapee_sample` jobs are queued later by
editor, profile-sampling, or data-integrity workflows with their own PP3
artifacts.
