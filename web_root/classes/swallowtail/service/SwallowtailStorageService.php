...
final class SwallowtailStorageService
{
    ...
    public function imagePath(string $storageBaseLocation, string $checksum, string $imageType): string
    {
        ...
        $extension = match ($imageType) {
            'source' => 'cr2',
            'source_profile', 'thumbnail_profile', 'preview_profile', 'final_profile' => 'pp3',
            default => 'jpg',
        };
    ...
    private function writableLocationsForChecksum(string $checksum, array $locations): array
    {
        ...
        if ((bool)\Swallowtail\Store\SwallowtailConfigurationStore::get('storage.round_robin_locations', false)) {
    ...
    private function mountedBaseLocations(): array
    {
        ...
        $testBaseLocation = trim((string)\Swallowtail\Store\SwallowtailConfigurationStore::get('storage.test_base_location', ''));
        if ($testBaseLocation !== '') {
        ...
        $includeRoot = (bool)\Swallowtail\Store\SwallowtailConfigurationStore::get('storage.store_on_root_partition', false);
